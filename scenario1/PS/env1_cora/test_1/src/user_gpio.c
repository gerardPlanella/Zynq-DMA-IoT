...
int user_gpio_get_fifo(FifoParams *fifo, char request){
	...
	if ((request & FIFO_UDF)){
		if(user_gpio_read_bit(FIFO_SIGNALS_DEVICE_ID, 2, (u32)FIFO_UDF_BIT, &(fifo->udf_err)) != XST_SUCCESS) return XST_FAILURE;
	}
	if ((request & FIFO_DATA_COUNT)){
		user_gpio_read(FIFO_DATA_DEVICE_ID, 1, &(fifo->data_count));
		fifo->data_count &= 0xFFF;
	}
...
int user_gpio_reset_fifo(void){
	...
	if(user_gpio_get_fifo(&fifo, FIFO_DATA_COUNT) != XST_SUCCESS) return XST_FAILURE;

	while(fifo.data_count);
